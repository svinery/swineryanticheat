...
{
    // pefile.ordlookup tablolarinin birebir kopyasi (otomatik uretildi).
    // Bazi DLL'ler (oleaut32, ws2_32/wsock32) fonksiyonlari ordinal ile import eder;
    // pefile bunlari gercek isimlere cevirir. imphash'in pefile ile ayni cikmasi
    // (genel tehdit-istihbarati feed'leriyle eslesmesi) icin ayni cevrimi yapariz.
    internal static class OrdinalLookup
    {
        ...
        // dllNameWithExt: uzanti dahil, kucuk harf (ornek "oleaut32.dll").
        // Bulamazsa null doner (cagiran taraf "ord{n}" kullanir).
        public static string? Resolve(string dllNameWithExt, int ordinal)
        {
            Dictionary<int, string>? table = dllNameWithExt switch
            {
            ...
            if (table != null && table.TryGetValue(ordinal, out string? name)) return name;
            return null;
        }
    }
}
